...
class ZK40 {
	...
	public function __construct($ip = null, $port = null, $protocol = 'UDP', $comPass=null)
	{
		if($ip != null)
		{
			$this->ip = $ip;
	...
	private function authKey($com_key, $session_id)
	{
		$key =$this->makeBaseKey($com_key, $session_id);
		$rand = rand(0, 255);
	...
	public function connect($ip = null, $port = 4370, $comPass=null)
	{
		if($comPass != null)
		{
			$this->comPass = $comPass;
		...
		try
		{
			...
			if(strlen($this->received_data)>0)
			{
				...
				$auth_key = $this->authKey($this->comPass, $this->session_id);
				$this->execCommand(CMD_AUTH, pack('I', $auth_key));			
		...
		try
		{
			...
			if(count($this->user_data) > 0)
			{
				...
				while(strlen($user_data) > 72)
				{
					...
					$users[$uid] = array('id'=>$userid, 'name'=>$name, 'role'=>intval($role), 'password'=>$password);
